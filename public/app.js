...
function __gripViewportPosition(claw,width,height){
  ...
  const centerY=r.top+r.height*.72;
...
function __prepareRealBall(target){
  ...
  return {target,pit,base:{x:r.left,y:r.top,width:r.width,height:r.height}};
}
function __moveRealBall(pickup,x,y,scale=.96){
  const dx=x-pickup.base.x;
  const dy=y-pickup.base.y;
...
async function __animateClawToWinner(winnerId){
  ...
  const start={x:pickup.base.x,y:pickup.base.y};
  let grip=__gripViewportPosition(claw,pickup.base.width,pickup.base.height);
  await __tween(420,p=>{
    __moveRealBall(pickup,start.x+(grip.x-start.x)*p,start.y+(grip.y-start.y)*p,1-.04*p);
  });
  grip=__gripViewportPosition(claw,pickup.base.width,pickup.base.height);
  __moveRealBall(pickup,grip.x,grip.y,.96);
  ...
  await __tween(1850,p=>{
    ...
    __moveRealBall(pickup,current.x,current.y,.96);
  });
  cable.style.height=`${baseCable}px`;
  claw.style.setProperty('--cable-length',`${baseCable}px`);
  ...
  __moveRealBall(pickup,finalGrip.x,finalGrip.y,.96);
}
