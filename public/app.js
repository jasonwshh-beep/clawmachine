...
function animateDraw(winnerId){
  ...
  schedule(()=>{claw.classList.add('is-dropping');claw.style.setProperty('--cable-length',`${cableLength}px`)},1250);
  schedule(()=>{claw.classList.remove('is-open');claw.classList.add('is-closed')},3000);
  schedule(()=>attachBallSmoothly(target,glass),3350);
  schedule(()=>{claw.classList.remove('is-dropping');claw.classList.add('is-lifting');claw.style.setProperty('--cable-length','78px')},4250);
  schedule(()=>{claw.style.left='50%'},6250);
}
