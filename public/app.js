...
function __gripViewportPosition(cargo,width,height){
  const r=cargo.getBoundingClientRect();
  return {x:r.left+r.width/2-width/2,y:r.top+r.height/2-height/2};
}
...
async function __animateClawToWinner(winnerId){
  ...
  let grip=__gripViewportPosition(cargo,pickup.start.width,pickup.start.height);
  await __tween(420,p=>{
  ...
  grip=__gripViewportPosition(cargo,pickup.start.width,pickup.start.height);
  ...
  await __tween(1800,p=>{
    ...
    cable.style.height=`${value}px`;
    claw.style.setProperty('--cable-length',`${value}px`);
    const current=__gripViewportPosition(cargo,pickup.start.width,pickup.start.height);
  ...
  claw.style.left='50%';
  await new Promise(requestAnimationFrame);
  const finalGrip=__gripViewportPosition(cargo,pickup.start.width,pickup.start.height);
