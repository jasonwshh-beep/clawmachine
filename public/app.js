...
function animateDraw(winnerId){
  ...
  const cableLength=Math.max(78,Math.min(g.height-210,targetCenter-165));
  claw.style.left=`${targetX}%`;
  schedule(()=>{claw.style.setProperty('--cable-length',`${cableLength}px`)},1750);
  ...
  schedule(()=>{claw.style.setProperty('--cable-length','78px')},4100);
