...
function animateDraw(winnerId){
  ...
  const clawWidth=claw.getBoundingClientRect().width;
  ...
  const gripOffset=108;
  const cableLength=Math.max(68,Math.min(g.height-190,targetCenterY-clawTop-gripOffset));
  ...
  schedule(()=>{claw.classList.remove('is-open');claw.classList.add('is-closed')},2920);
  schedule(()=>captureBall(target,glass),3150);
  schedule(()=>{
    claw.classList.remove('is-dropping');claw.classList.add('is-lifting');
