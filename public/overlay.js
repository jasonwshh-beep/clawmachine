...
function captureBall(target,glass){
  ...
  clone.removeAttribute('id');clone.classList.remove('is-picked');clone.classList.add('pickup-ball');
  Object.assign(clone.style,{left:`${targetRect.left-glassRect.left}px`,top:`${targetRect.top-glassRect.top}px`,width:`${targetRect.width}px`,height:`${targetRect.height}px`,transition:'none',transform:'none'});
  glass.appendChild(clone);target.classList.add('is-picked');
  ...
  clone.animate([{left:`${targetRect.left-glassRect.left}px`,top:`${targetRect.top-glassRect.top}px`,transform:'scale(1)'},{left:`${endLeft}px`,top:`${endTop}px`,transform:'scale(.96)'}],{duration:360,easing:'cubic-bezier(.2,.8,.2,1)',fill:'forwards'}).finished.then(()=>{
    cargo.style.setProperty('--captured-size',`${Math.min(60,targetRect.width)}px`);clone.classList.remove('pickup-ball');clone.removeAttribute('style');cargo.appendChild(clone);
  }).catch(()=>{});
}
function animate(winnerId){
  clearAnim();$('#winnerCard').classList.remove('show');
  const glass=$('#glassPanel'),target=$(`#ball-${CSS.escape(String(winnerId))}`),claw=$('#claw');
  if(!glass||!target)return;
  const g=glass.getBoundingClientRect(),b=target.getBoundingClientRect(),clawWidth=claw.getBoundingClientRect().width;
  const targetCenterX=b.left+b.width/2-g.left;
  const clampedX=Math.max(clawWidth*.52,Math.min(g.width-clawWidth*.52,targetCenterX));
  const targetCenterY=b.top+b.height/2-g.top;
  const clawTop=parseFloat(getComputedStyle(claw).top)||28;
  const cableLength=Math.max(68,Math.min(g.height-190,targetCenterY-clawTop-108));
  claw.getAnimations().forEach(a=>a.cancel());
  const fromLeft=claw.getBoundingClientRect().left+clawWidth/2-g.left;
  const move=claw.animate([{left:`${fromLeft}px`},{left:`${clampedX}px`}],{duration:1250,easing:'cubic-bezier(.45,.05,.18,1)',fill:'forwards'});
  move.finished.then(()=>{claw.style.left=`${clampedX}px`;move.cancel();}).catch(()=>{});
  later(()=>{claw.classList.add('is-dropping');claw.style.setProperty('--cable-length',`${cableLength}px`)},1320);
  later(()=>{claw.classList.remove('is-open');claw.classList.add('is-closed')},2920);
  later(()=>captureBall(target,glass),3150);
  later(()=>{claw.classList.remove('is-dropping');claw.classList.add('is-lifting');claw.style.setProperty('--cable-length','68px')},3650);
  later(()=>{
    const current=parseFloat(claw.style.left)||clampedX;
    const back=claw.animate([{left:`${current}px`},{left:`${g.width/2}px`}],{duration:1200,easing:'cubic-bezier(.45,.05,.18,1)',fill:'forwards'});
    back.finished.then(()=>{claw.style.left='50%';back.cancel()}).catch(()=>{});
  },5300);
}
