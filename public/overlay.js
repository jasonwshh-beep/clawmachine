...
function clearAnim(){
  ...
  $('#clawCargo').innerHTML='';
  document.querySelectorAll('.pickup-ball,.pickup-ball-fixed').forEach(el=>el.remove());
}
...
function __makeViewportPickup(target){
  const r=target.getBoundingClientRect();
  const clone=target.cloneNode(true);
  clone.removeAttribute('id');
  clone.classList.remove('is-picked','pickup-ball');
  clone.classList.add('pickup-ball-fixed');
  clone.style.setProperty('--pickup-x',`${r.left}px`);
  clone.style.setProperty('--pickup-y',`${r.top}px`);
  clone.style.setProperty('--pickup-width',`${r.width}px`);
  clone.style.setProperty('--pickup-height',`${r.height}px`);
  clone.style.setProperty('--pickup-scale','1');
  document.body.appendChild(clone);
  target.classList.add('is-picked');
  return {clone,start:{x:r.left,y:r.top,width:r.width,height:r.height}};
}
function __gripViewportPosition(claw,width,height){
  // Follow the visible claw itself, not an independently positioned cargo marker.
  // The cable is part of the claw's normal layout, so the claw's bottom edge moves
  // continuously as the cable extends and retracts.
  const r=claw.getBoundingClientRect();
  const scale=Math.max(.55,Math.min(1.25,r.width/(claw.offsetWidth||r.width||1)));
  const gripCenterX=r.left+r.width/2;
  // Place the ball between the curled tips, slightly above the gripper's bottom.
  const gripCenterY=r.bottom-Math.max(12,18*scale)-height*.36;
  return {x:gripCenterX-width/2,y:gripCenterY-height/2};
}
function __setViewportPickup(ball,x,y,scale=.96){
  ball.style.setProperty('--pickup-x',`${x}px`);
  ball.style.setProperty('--pickup-y',`${y}px`);
  ball.style.setProperty('--pickup-scale',String(scale));
}
async function __animateClawToWinner(winnerId){
  ...
  const pickup=__makeViewportPickup(target);
  let grip=__gripViewportPosition(claw,pickup.start.width,pickup.start.height);
  await __tween(420,p=>{
    const x=pickup.start.x+(grip.x-pickup.start.x)*p;
    const y=pickup.start.y+(grip.y-pickup.start.y)*p;
    __setViewportPickup(pickup.clone,x,y,1-.04*p);
  });
  grip=__gripViewportPosition(claw,pickup.start.width,pickup.start.height);
  __setViewportPickup(pickup.clone,grip.x,grip.y,.96);
  ...
  await __tween(1800,p=>{
    ...
    cable.style.height=`${value}px`;
    claw.style.setProperty('--cable-length',`${value}px`);
    const current=__gripViewportPosition(claw,pickup.start.width,pickup.start.height);
    __setViewportPickup(pickup.clone,current.x,current.y,.96);
  });
  cable.style.height=`${baseCable}px`;
  claw.style.setProperty('--cable-length',`${baseCable}px`);
  ...
  claw.style.left='50%';
  await new Promise(requestAnimationFrame);
  const finalGrip=__gripViewportPosition(claw,pickup.start.width,pickup.start.height);
  __setViewportPickup(pickup.clone,finalGrip.x,finalGrip.y,.96);
}
