...
async function captureBall(target,glass){
  ...
  const startLeft=targetRect.left-glassRect.left;
  const startTop=targetRect.top-glassRect.top;
  ...
  await clone.animate([
    {left:`${startLeft}px`,top:`${startTop}px`,transform:'translate3d(0,0,0) scale(1)'},
    {left:`${endLeft}px`,top:`${endTop}px`,transform:'translate3d(0,0,0) scale(.96)'}
  ],{
    duration:520,
...
async function __animateClawToWinner(winnerId){
  ...
  await captureBall(target,glass);
  await __sleep(180);

  claw.classList.remove('is-dropping');
  claw.classList.add('is-lifting');
  const cargo=document.querySelector('#clawCargo');
  const cargoStart=parseFloat(getComputedStyle(cargo).top)||desiredCable+78;
  const cargoEnd=68+78;
  await Promise.all([
    cable.animate(
