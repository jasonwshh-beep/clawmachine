...
function makeEntrant(username, avatarUrl) {
  ...
  return {
    id: crypto.randomUUID(), username, avatarUrl: avatarUrl || '',
  ...
  try {
    ...
    ws.on('message', raw=>{
      try {
        ...
        const avatarUrl=sender?.profile_pic || sender?.profile_picture || sender?.avatar || '';
        processChatMessage(cleanName(sender.username), String(data?.content || ''), avatarUrl);
