...
app.post('/api/admin/entry', admin, (req,res)=>{
  const wasOpen = state.entriesOpen;
  state.entriesOpen = true;
  const result = addEntrant(req.body.username, req.body.avatarUrl);
  state.entriesOpen = wasOpen;
  if (!result.ok) return res.status(400).json({error:result.reason || 'Could not add entry'});
  broadcast();
  res.json(result);
});
...
app.post('/api/admin/draw', admin, (_,res)=>{
  ...
  io.emit('draw:start', {winnerId:winner.id, duration:9000});
  setTimeout(()=>{
